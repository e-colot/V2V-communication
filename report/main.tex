...
\section{Optimal carrier frequency}
In section \ref{sec:transfer_function}, it was shown that the transfer function is not constant and that some frequencies are more attenuated than others. A frequency was pointed to be less attenuated than $5.9$ GHz but as the analysis was done for a distance of 500m, the conclusion was that this frequency might not be optimal for other distances. \\
The objective of this section is to find the optimal carrier frequency over all the distances. The analysis is done by computing the transfer function for every distance between 0 and 1000m. The frequencies are limited between $5.5$ and $6.5$ GHz to have a better view of the results. The amplitude of the transfer function in dB is shown in figure \ref{fig:transfer_function_2}.\\

\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{6_3.eps}
    \caption{Transfer function of the wideband channel with multiple rays for a varying distance}
    \label{fig:transfer_function_2}
\end{figure}

It is difficult to see the optimal frequency on this plot so the mean of the transfer function over the distance is computed and shown in figure \ref{fig:optimal_frequency}. The optimal frequency is the one that has the highest mean amplitude.

\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{6_4.eps}
    \caption{Mean transfer function of the wideband channel with multiple rays for a varying distance}
    \label{fig:optimal_frequency}
\end{figure}

Choosing the optimal frequency is not an easy task\footnote{The regulations must also be taken into account but this is neglected here as only the physics is discussed} and taking the highest mean amplitude is a simplistic method. It gave as optimal frequency:
\begin{equation*}
    f_{\text{opt, mean}} = 5.987 \text{GHz}
\end{equation*}

A more advanced method is proposed next and it consists of a 4-step process:\\
First, the transfer function is corrected to take the attenuation due to the distance into account. From equation \ref{eq:transfer_function}, the transfer function is inversely proportional to the distance. The correction is done by multiplying the transfer function by $d$ and the result is shown in figure \ref{fig:transfer_function_corrected}. This allows each distance to have the same weight in the computation of the optimal frequency.

\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{6_5.eps}
    \caption{Transfer function of the wideband channel with multiple rays for a varying distance, corrected for the distance attenuation, which makes the transfer function flatter over the distances}
    \label{fig:transfer_function_corrected}
\end{figure}

Second, the minimum of the transfer function is computed across all frequencies, and only the frequencies with values in the top 10\% of that range are retained.\\

Third, to avoid having a channel that has has an attenuation that changes too much with the distance, the standard deviation of the transfer function is computed for each remaining frequency and the frequencies with a standard deviation in the top 90\% are removed.\\

Finally, the one with the highest maximal value is chosen as the optimal frequency. This gives:
\begin{equation*}
    f_{\text{opt, combination}} = 5.819 \text{GHz}
\end{equation*}

This method is more complex but it allows to have a result that takes more parameters into account. If the carrier frequency was a free parameter, this would be the one to choose according to these criterions. \\
